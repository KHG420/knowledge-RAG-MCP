...
跨文档语义与关键词搜索。系统自动处理：知识库路由、查询理解、关键词/语义扩展、
检索策略选择（BM25 / 向量 / 混合）、重排序、证据置信度评分。

**直接传入用户的原始问题即可——不要手动重写成关键词，不要预先选择模式或 KB。**
内部查询分析器会自动处理一切。

| 参数 | 必填 | 说明 |
|-----------|----------|-------------|
| `question` | **是** | 用户的原始自然语言问题。直接传入原文——内部查询分析器会自动处理中英文扩展和领域术语 |
| `search_keywords` | 否 | **已废弃** — 请用 `question` 替代。仅为兼容旧版 Agent 保留 |
| `kbName` | 否 | 可选。留空即可——系统自动路由到最佳的 1～3 个知识库。仅在需要强制指定某个 KB 时传入 |
| `limit` | 否 | 最大结果数（默认 8，上限 20） |
| `mode` | 否 | **已废弃** — 系统自动选择最佳策略。`bm25` 或 `hybrid`，仅为兼容旧版保留 |
...
| `coarse` | 否 | 启用粗到细两阶段搜索：先对章节打分，再仅在 top-3 章节内搜索 |
